  ...
  <div class="cards-objects">
    <a-card :extra="extra" style="width: 100%;" @click="goToDetails">
      <div class="grid grid-col-cast gap-5">
        ...
        <div class="flex flex-col justify-center">
          <span v-if="object.highway.value">{{ `${object.highway.value}` }} ш.</span></div>
        <div class="flex flex-col justify-center">
          <span v-if="object.distance_from_mkad.value">{{ object.distance_from_mkad.value + ' км' }}</span></div>
        <div class="flex flex-col justify-center">
          <span class="my-auto" v-if="object.place.value">{{ `${object.place.value}` }}</span>
        </div>
        ...
        <div class=" flex flex-col justify-center">
          ...
          <div class="my-auto" v-else>Не опубликован</div>
        </div>
      </div>
    </a-card>
  </div>
